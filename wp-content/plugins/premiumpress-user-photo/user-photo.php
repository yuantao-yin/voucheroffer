<?php
/*
Plugin Name: PremiumPress User Photo
Plugin URI: http://www.premiumpress.com
Description: Allows users to associate photos with their accounts by accessing their "Your Profile" page.
Version: 1
Author: 
*/
...
function userphoto__get_userphoto($user_id, $photoSize, $before, $after, $attributes, $default_src){
	...
	if($user_id && ($userdata = get_userdata($user_id))){
		    ...
		    $image_file = ($photoSize == USERPHOTO_FULL_SIZE ? $userdata->userphoto_image_file : $userdata->userphoto_thumb_file))
		{
			...
			$src = trailingslashit($upload_dir['baseurl']) . $image_file;
			$src = str_replace("uploads/","themes/".strtolower(constant('PREMIUMPRESS_SYSTEM'))."/thumbs/",$src);
		...
		if(empty($attributes['width']) && !empty($width))
			$img .= ' style="max-width:' . htmlspecialchars($width) . 'px';
		if(empty($attributes['height']) && !empty($height))
			$img .= ' max-height:' . htmlspecialchars($height) . 'px';
...
//		$img = '<img src="' . get_option('siteurl') . '/wp-content/uploads/userphoto1/' . $userdata->userphoto_image_file . '"';
//		$img .= ' alt="' . htmlspecialchars($userdata->display_name) . '"';
...
//		$img = '<img src="' . get_option('siteurl') . '/wp-content/uploads/userphoto1/' . $userdata->userphoto_thumb_file . '"';
//		$img .= ' alt="' . htmlspecialchars($userdata->display_name) . '"';
...
global $wpdb;
	...
	if(@$_POST['userphoto_delete']){
		...
		if($userdata->userphoto_image_file){
			$upload_dir = wp_upload_dir();
			$bdir = trailingslashit(get_option('imagestorage_path'));
			$bdir = str_replace("uploads/","themes/".strtolower(constant('PREMIUMPRESS_SYSTEM'))."/thumbs/",$bdir);
	...
	#Upload photo or change approval status
		...
		if(isset($_FILES['userphoto_image_file']) && @$_FILES['userphoto_image_file']['name']){
			...
			$thumbinfo = null;
			if(!$error){
				$userphoto_maximum_dimension = 200;
			...
			if(!$error){
				$upload_dir = wp_upload_dir();
				$dir = trailingslashit(get_option('imagestorage_path'));
				$dir = str_replace("uploads/","themes/".strtolower(constant('PREMIUMPRESS_SYSTEM'))."/thumbs/",$dir);
				...
				if(!$error){
					...
					if(!move_uploaded_file($tmppath, $imagepath)){
						...
						#Update usermeta
							...
							update_usermeta($userID, "userphoto_approvalstatus", '2');
...
function userphoto_display_selector_fieldset(){
	...
	<?php endif; ?>
        ...
        <?php if($profileuser->userphoto_image_file): ?>
			<?php
			$upload_dir = wp_upload_dir();
			$bdir = trailingslashit($upload_dir['baseurl']);
			$bdir = str_replace("uploads/","themes/".strtolower(constant('PREMIUMPRESS_SYSTEM'))."/thumbs/",$bdir);
		...
		<?php endif; ?>
        <p id='userphoto_image_file_control' class="f_half left">
...
function userphoto_options_page(){
	#Get option values
	$userphoto_jpeg_compression = "100";//get_option( 'userphoto_jpeg_compression' );
	$userphoto_maximum_dimension = "150";//get_option( 'userphoto_maximum_dimension' );
	$userphoto_thumb_dimension = "80";//get_option( 'userphoto_thumb_dimension' );
	$userphoto_admin_notified = "0";//get_option( 'userphoto_admin_notified' );
	$userphoto_level_moderated = "2";//get_option( 'userphoto_level_moderated' );
	$userphoto_use_avatar_fallback = "Y";//get_option('userphoto_use_avatar_fallback');
	$userphoto_override_avatar = "Y";//get_option('userphoto_override_avatar');
